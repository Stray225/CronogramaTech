...
function ShiftCell({ date, employeeId, assignment, onShiftChange, dimmed = false }: ShiftCellProps) {
  ...
  return (
    <td
      className={`relative p-0 border-r border-slate-100 dark:border-slate-700/60 last:border-r-0 ${dimmed ? "opacity-30" : ""}`}
    >
      {/* ── Shift chip ── */}
      <div
        role="button"
        tabIndex={0}
        aria-label={`Turno: ${shift.label}${assignment.note ? `, nota: ${assignment.note}` : ""}. Clic para cambiar.`}
        aria-expanded={open}
        ...
        className={`
          ...
          px-1 py-3 cursor-pointer select-none
          min-h-[52px] flex flex-col items-center justify-center gap-1
          transition-all duration-100
          hover:brightness-[0.93] dark:hover:brightness-125
          ${open ? "ring-2 ring-inset ring-indigo-500 dark:ring-indigo-400" : ""}
        `}
      >
        <span className="leading-tight">{shift.label}</span>
        {assignment.note && (
          <span className="text-[10px] font-normal opacity-75 truncate max-w-[100px] leading-none">
            {assignment.note}
          </span>
        )}
      </div>

      {/* ── Dropdown — opens UPWARD to never overlap rows below ── */}
      {open && (
        <div
          ref={ref}
          role="listbox"
          aria-label="Seleccionar turno"
          className="absolute z-50 bottom-full left-0 mb-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl w-52 overflow-hidden animate-slide-down"
        >
              ...
              <button
                ...
                className={`flex items-center gap-2.5 w-full text-left px-3 py-2 text-xs transition-colors
                  ...
                  ${assignment.shift === s.code
                    ? "font-bold text-slate-900 dark:text-slate-100 bg-slate-100 dark:bg-slate-700"
                    : "text-slate-700 dark:text-slate-300"}`}
              >
                <span className={`inline-block w-3 h-3 rounded-full flex-shrink-0 ${s.color}`} />
                <span className="flex-1">{s.label}</span>
                {s.hours > 0 && (
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 font-normal tabular-nums">
                    {s.hours}h
                  </span>
                )}
              </button>
